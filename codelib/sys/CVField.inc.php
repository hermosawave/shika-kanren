...
/////
   ...
   */
	...
	// Error Handling
	//----------------------------------
	function SetPDErrMsg( $err_msg_id, $v = null )
	{
		$b = ( $this->Get(XA_HIDE_VALUE) == '' ? false : $this->Get(XA_HIDE_VALUE) );
		$this->SetErrMsg( $this->sys->Error->GetPDErrMsg( $err_msg_id, $v, $b ) );
	}
   ...
   */
	...
	{
		$c = $this->GetCaption();
		$err_msg = CMBStr::replace( '##c##', $c, $err_msg );
   ...
   */
	function Validate_MinChar( $v )
	{
		...
		{
			$this->SetPDErrMsg( EMT_TOO_SHORT, $v );
			return false;
		}
   ...
   */
	function Validate_MaxChar( $v )
	{
		...
		{
			$this->SetPDErrMsg( EMT_TOO_LONG, $v );
			return false;
		}
...
class CVNumber extends CVField
{
   ...
   */
	function Validate_MinNum( $v, $min = null )
	{
		...
		{
			$this->SetPDErrMsg( EMT_TOO_SMALL, $v );
			return false;
		}
   ...
   */
	function Validate_MaxNum( $v, $max = null )
	{
		...
		{
			$this->SetPDErrMsg( EMT_TOO_LARGE, $v );
			return false;
		}
...
class CVAsciiText extends CVText
{
   ...
   */
	...
	{
		//if ( !ft_preg_match( $v, PREG_ALPHABET_NAME ) )
		if ( !ereg("^([\x09,\x0a,\x0c,\x20-\x7e]*)$", $v) )
		{
			$this->SetPDErrMsg( EMT_NOT_ASCII, $v );
			return false;
		}
...
class CVDigit extends CVText
{
   ...
   */
	function Validate_Digit( $v )
	{
		...
		if ( !ereg("^([0-9]*)$", $v) )
		{
			$this->SetPDErrMsg( EMT_NOT_DIGIT, $v );
			return false;
		}
