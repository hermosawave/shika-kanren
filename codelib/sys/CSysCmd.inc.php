...
class CSysCmd
{
	...
	function SetNextSc( $syscmd )
	{
		...
		{
			$ax = explode( STR_SC_INTERNAL_SEPA, $syscmd );
			if ( count($ax) == 2 )
			{
	...
	function OutputPage( $path_template )
	{
		...
		$ienc = CConfig::Get( 'system/encoding/internal' );
		$oenc = CConfig::Get( 'system/encoding/output' );
		$b_change_char_encoding = ( $ienc != $oenc );
		//-- [END] Character-Encoding

		//-- [BEGIN] Mockup
		$b_print_to_html = CConfig::Get("debug/print_to_html");
		//-- [END] Mockup
...
<title>ERROR</title>
<body><div style='color:red'>Critical Error : {$msg}</div>
</body></html>
_EOM_;
		echo $txt;
		die;
	...
	{
		$msg = RSTR_ERR_DOUBLE_SUBMIT;
		$txt =<<<_EOM_
<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>ERROR</title>
<body><div style='color:red'>{$msg}</div>
</body></html>
_EOM_;
		echo $txt;
		die;
