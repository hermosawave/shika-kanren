...
class CEmail
{
	...
	function OpenConfig( $path, $b_direct_config = false )
	{
		...
		foreach ( $ls as $key )
		{
			...
			{
				$delimiter = "~~~~~";
				$ax = CMBStr::explode( ",", str_replace( ",,", $delimiter, $params[$key] ) );
				$ax2= array();
				foreach ( $ax as $s )
				{
					$s = trim( $s );
					...
					foreach ( $bx as $ss )
					{
						$bx2[] = str_replace( $delimiter, ",", trim( $ss ) );
					}
					$ax2[] = $bx2;
				}
				$params[$key] = $ax2;
	...
	function get_folder_path( $path )
	{
		...
		return $dirname . '/';
/*
		$pos1 = CMBStr::strrpos( $path, "/" );
		$pos2 = CMBStr::strrpos( $path, "\\" );
		if (( $pos1 === false ) && ( $pos2 === false ))
			...
			$pos = $pos2;
		$path0 = CMBStr::substr( $path, 0, $pos ) . "/";
		return $path0;
*/
	}
	...
	function IsAscii( $s )
	{
		...
		return mb_ereg_match( "^[a-zA-Z0-9_ ]*$", $s );
	}
